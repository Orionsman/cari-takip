...
DB_FILE = os.environ.get("DB_PATH", os.path.join(os.path.dirname(__file__), "cari_hesap.db"))
...
def api_cari_ozet(cid):
    ...
    b, a = r["borc"], r["alacak"]
    return jsonify({"borc": b, "alacak": a, "bakiye": b - a})
...
def api_hareketler(cid):
    ...
    result, bakiye = [], 0
    for r in rows:
        r = dict(r); bakiye += r["borc"] - r["alacak"]; r["bakiye"] = bakiye; result.append(r)
...
def api_satis_sil(sid):
    with get_db() as conn:
        r = conn.execute("SELECT cari_id,tarih,toplam FROM satislar WHERE id=?", (sid,)).fetchone()
        if r:
            conn.execute(
                "DELETE FROM hareketler WHERE cari_id=? AND tarih=? AND borc=? AND tur='satış'",
                (r["cari_id"], r["tarih"], r["toplam"]))
...
def api_odeme_sil(oid):
    with get_db() as conn:
        r = conn.execute("SELECT cari_id,tarih,tutar FROM odemeler WHERE id=?", (oid,)).fetchone()
        if r:
            conn.execute(
                "DELETE FROM hareketler WHERE cari_id=? AND tarih=? AND alacak=? AND tur='ödeme'",
                (r["cari_id"], r["tarih"], r["tutar"]))
...
init_db()  # gunicorn için de çalışsın
